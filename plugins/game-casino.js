...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
    ...
    if (user.limit >= count * 1) {
        user.limit -= count * 1
        if (Aku > Kamu) {
            conn.reply(m.chat, '`🍟 Veamos que numeros tienen!`\n\n' + `➠ *🍫 𝑺𝑰𝑺𝑲𝑬𝑫 𝑩𝑶𝑻 - 𝑴𝑫* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} 🪙 Creds.`.trim(), m,)
        } else if (Aku < Kamu) {
            user.limit += count * 2
            conn.reply(m.chat, '`🍟 Veamos que numeros tienen!`\n\n' + `➠ *🍫 𝑺𝑰𝑺𝑲𝑬𝑫 𝑩𝑶𝑻 - 𝑴𝑫* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *GANASTE* ${formatNumber(count * 2)} 🪙 Creds.`.trim(), m,)
        } else {
            user.limit += count * 1
            conn.reply(m.chat, '`🍟 Veamos que numeros tienen!`\n\n' + `➠ *🍫 𝑺𝑰𝑺𝑲𝑬𝑫 𝑩𝑶𝑻 - 𝑴𝑫* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username} obtienes ${formatNumber(count * 1)} 🪙 Creds.`.trim(), m,)
        }
    } else conn.reply(m.chat, `No tienes *${formatNumber(count)} 🪙 Creds* para apostar!`.trim(), m,)
